-- Remove house edge: winner takes ALL loot rolled across both/all teams
ALTER TABLE public.case_battles ALTER COLUMN house_edge_bps SET DEFAULT 0;
UPDATE public.case_battles SET house_edge_bps = 0 WHERE status = 'waiting';
...
BEGIN
  ...
  FOR cs IN SELECT * FROM public.battle_cases WHERE battle_id = _battle_id ORDER BY position LOOP
    ...
    FOR pl IN SELECT * FROM public.battle_players WHERE battle_id = _battle_id ORDER BY slot LOOP
      v_special := 'none';
      IF random() < 0.04 THEN v_special := 'empire'; END IF;
      IF v_special = 'none' AND random() < 0.06 THEN v_special := 'duel'; END IF;

      IF v_special = 'empire' THEN
        -- Empire: re-roll, take the better of two pulls (upgraded pool feel)
  ...
  -- Winner takes EVERYTHING: full sum of all rolls across all players, no house edge.
  ...
  BEGIN
    ...
    IF human_count > 0 AND v_payout > 0 THEN
