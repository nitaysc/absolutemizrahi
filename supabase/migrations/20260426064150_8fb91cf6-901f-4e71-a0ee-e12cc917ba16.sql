DROP FUNCTION IF EXISTS public.pump_cashout();
DROP FUNCTION IF EXISTS public.pump_pump();
...
BEGIN
  ...
  _step := CASE _diff
    WHEN 'easy'   THEN 1.0175
    WHEN 'medium' THEN 1.065
    WHEN 'hard'   THEN 1.16
  ...
  IF _pumps >= _pop_at THEN
    ...
    INSERT INTO public.bets (user_id, game, bet_amount, payout, multiplier, won, details)
      VALUES (_uid, 'pump', _bet, 0, 0, false,
              jsonb_build_object('difficulty', _diff, 'pop_at', _pop_at, 'pumps', _pumps));
...
RETURNS TABLE(payout bigint, multiplier numeric, new_balance bigint, pop_at integer)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  IF _pumps < 1 THEN RAISE EXCEPTION 'Pump at least once'; END IF;
  ...
  RETURN QUERY SELECT _payout, _mult, _bal, _pop_at;
END;
$function$;