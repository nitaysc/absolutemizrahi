...
DECLARE
  ...
  v_winner_team int; v_max bigint; v_min bigint;
  v_payout bigint; team_count int;
...
BEGIN
  ...
  -- Award all rolled items from the winning team to winning humans as inventory.
  -- Items are split equally across humans on the winning team (round-robin by slot).
  -- Borrowed amounts are deducted by selling items lowest-value first until repaid.
  DECLARE
    human_ids uuid[]; human_count int; idx int := 0;
    r record; target_user uuid; w record; debt bigint; sold_for bigint;
    inv_id uuid;
  BEGIN
    SELECT array_agg(user_id ORDER BY slot) INTO human_ids
    FROM public.battle_players
    WHERE battle_id = _battle_id AND team = v_winner_team AND is_bot = false;

    human_count := COALESCE(array_length(human_ids, 1), 0);

    IF human_count > 0 THEN
      -- Distribute every round's item from winning-team slots round-robin to winning humans.
      FOR r IN
        SELECT br.*, bp.user_id AS slot_user, bp.is_bot
        FROM public.battle_rounds br
        JOIN public.battle_players bp
          ON bp.battle_id = br.battle_id AND bp.slot = br.player_slot
        WHERE br.battle_id = _battle_id AND bp.team = v_winner_team
        ORDER BY br.round_index, br.player_slot
      LOOP
        target_user := human_ids[(idx % human_count) + 1];
        idx := idx + 1;
        INSERT INTO public.inventory(user_id, item_name, item_image, value, rarity, source, source_ref, status)
        VALUES (target_user, r.item_name, r.item_image, r.item_value, r.rarity, 'battle', _battle_id, 'held');
      END LOOP;

      -- Repay borrowed amounts: auto-sell cheapest battle items from this battle until debt cleared.
      FOR w IN SELECT * FROM public.battle_players
        WHERE battle_id = _battle_id AND team = v_winner_team AND is_bot = false AND borrowed > 0
      LOOP
        debt := w.borrowed;
        FOR inv_id, sold_for IN
        ...
        LOOP
          ...
          SET status = 'sold', sold_for = sold_for, sold_at = now()
          WHERE id = inv_id;
          -- The repayment value is consumed by the loan; nothing is credited to coins.
          debt := debt - sold_for;
        END LOOP;
      END LOOP;
    END IF;
