...
DECLARE
  ...
  v_winner_team int;
  v_payout bigint; team_count int; team_size_actual int;
  share bigint; net_share bigint;
...
BEGIN
  ...
  share := v_payout / team_size_actual;
  ...
  -- Distribute ALL items rolled by the winning team round-robin among winning HUMANS (shared loot).
  -- Each winning human also receives their pot share (1 / team_size of payout) minus any borrow.
  DECLARE
    human_ids uuid[]; human_count int; idx int := 0;
    r record; target_user uuid; w record; debt bigint;
  ...
  BEGIN
    ...
    IF human_count > 0 THEN
      -- Share items round-robin among winning humans
      FOR r IN
        SELECT br.*
      ...
      -- Pay each winning human their pot share, less borrow
      FOR w IN SELECT * FROM public.battle_players
        WHERE battle_id = _battle_id AND team = v_winner_team AND is_bot = false
      LOOP
        net_share := GREATEST(share - w.borrowed, 0);
        IF net_share > 0 THEN
          UPDATE public.profiles SET coins = coins + net_share, total_won = total_won + net_share
          WHERE id = w.user_id;
        END IF;

        IF w.borrowed > share THEN
          debt := w.borrowed - share;
