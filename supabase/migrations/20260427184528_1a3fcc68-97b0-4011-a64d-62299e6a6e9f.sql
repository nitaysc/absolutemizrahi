-- Loosen the check constraints so newer modes/statuses are accepted
ALTER TABLE public.case_battles DROP CONSTRAINT IF EXISTS case_battles_status_check;
ALTER TABLE public.case_battles ADD CONSTRAINT case_battles_status_check
  CHECK (status = ANY (ARRAY['waiting'::text, 'running'::text, 'rolling'::text, 'finished'::text, 'cancelled'::text]));

ALTER TABLE public.case_battles DROP CONSTRAINT IF EXISTS case_battles_type_check;
ALTER TABLE public.case_battles ADD CONSTRAINT case_battles_type_check
  CHECK (type = ANY (ARRAY['normal'::text, 'crazy'::text, 'group'::text, 'terminal'::text, 'empire'::text, 'duel'::text]));

-- Restore auto-fill-with-bots inside start_case_battle (the latest version
-- accidentally dropped this), and keep inventory payout behavior.
...
DECLARE
  ...
  team_count int;
  pcount int; bot_n int := 1; s int; v_team int;
...
BEGIN
  ...
  -- Fill empty slots with bots if the battle is set to auto-fill,
  -- otherwise require the room to be full.
