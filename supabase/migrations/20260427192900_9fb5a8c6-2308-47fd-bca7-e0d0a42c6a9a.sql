...
ALTER TABLE public.case_battles
  ADD COLUMN IF NOT EXISTS allow_borrow boolean NOT NULL DEFAULT false;

ALTER TABLE public.battle_players
  ADD COLUMN IF NOT EXISTS borrowed bigint NOT NULL DEFAULT 0;

-- Recreate create_case_battle with _allow_borrow
...
DECLARE
  ...
  v_user_balance bigint; v_pay bigint; v_borrow bigint := 0;
BEGIN
  ...
  IF v_user_balance < v_total THEN
    IF NOT _allow_borrow THEN RAISE EXCEPTION 'Insufficient coins'; END IF;
    v_pay := GREATEST(v_user_balance, 0);
    v_borrow := v_total - v_pay;
  ELSE
    v_pay := v_total;
    v_borrow := 0;
  END IF;
...
-- join_case_battle with borrow support
CREATE OR REPLACE FUNCTION public.join_case_battle(_battle_id uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_uid uuid := auth.uid();
  b record; v_slot int; v_team int; v_balance bigint; v_pay bigint; v_borrow bigint := 0;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  ...
  IF v_balance < b.per_player_cost THEN
    IF NOT b.allow_borrow THEN RAISE EXCEPTION 'Insufficient coins'; END IF;
    v_pay := GREATEST(v_balance, 0);
    v_borrow := b.per_player_cost - v_pay;
  ELSE
    v_pay := b.per_player_cost;
    v_borrow := 0;
  END IF;
...
BEGIN
  ...
  IF b.host_id = v_uid THEN
    -- refund any remaining players what they actually paid, then cancel
...
-- start_case_battle: deduct borrowed from each winner's share before crediting (cap at 0)
...
BEGIN
  ...
  -- Distribute payout equally; for each winner deduct their borrowed loan first,
  -- then recover the rest from their current balance but never go below 0.
  DECLARE
    human_count int; share bigint; w record; net_share bigint;
    debt_left bigint; bal bigint; take_from_bal bigint;
  BEGIN
    ...
    IF human_count > 0 THEN
      ...
      FOR w IN SELECT * FROM public.battle_players
        WHERE battle_id = _battle_id AND team = v_winner_team AND is_bot = false
      LOOP
        IF share >= w.borrowed THEN
          net_share := share - w.borrowed;
          UPDATE public.profiles SET coins = coins + net_share,
            total_won = total_won + net_share WHERE id = w.user_id;
        ELSE
          -- share doesn't cover loan; try to take rest from balance, capped at 0
          debt_left := w.borrowed - share;
          SELECT coins INTO bal FROM public.profiles WHERE id = w.user_id FOR UPDATE;
          take_from_bal := LEAST(GREATEST(bal,0), debt_left);
          UPDATE public.profiles SET coins = coins - take_from_bal WHERE id = w.user_id;
        END IF;
      END LOOP;
    END IF;

    -- Losers with debt: take what we can from balance, cap at 0 (no negative)
    FOR w IN SELECT * FROM public.battle_players
      WHERE battle_id = _battle_id AND is_bot = false AND borrowed > 0
        AND team <> v_winner_team
    LOOP
      SELECT coins INTO bal FROM public.profiles WHERE id = w.user_id FOR UPDATE;
      take_from_bal := LEAST(GREATEST(bal,0), w.borrowed);
      IF take_from_bal > 0 THEN
        UPDATE public.profiles SET coins = coins - take_from_bal WHERE id = w.user_id;
      END IF;
    END LOOP;
  END;
