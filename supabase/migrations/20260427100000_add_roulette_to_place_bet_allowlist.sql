-- Allow roulette bets to settle through place_bet without raising "Unknown game".
...
DECLARE
  _uid UUID := auth.uid();
  _bal BIGINT; _payout BIGINT; _profit BIGINT; _bid UUID;
BEGIN
  IF _uid IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF _bet_amount <= 0 THEN RAISE EXCEPTION 'Bet must be positive'; END IF;
  IF _game NOT IN ('dice','coinflip','limbo','chicken','plinko','pump','snakes','moles','prediction','roulette') THEN
