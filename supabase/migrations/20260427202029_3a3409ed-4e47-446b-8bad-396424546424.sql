...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      given := 0;
      -- Award this player's own rolled items, cheapest first, up to their share value
      FOR r IN
      ...
      -- Repay borrow first from voucher remainder
      debt := w.borrowed;
      ...
      -- Award leftover as a sellable Money Voucher item in inventory
      IF remainder > 0 THEN
        INSERT INTO public.inventory(user_id, item_name, item_image, value, rarity, source, source_ref, status)
