...
BEGIN
  ...
  -- NEW: payout = sum of items rolled across ALL players (the actual loot pile),
  -- minus the 5% house edge. Loser teams forfeit their items to the winner.
  SELECT COALESCE(SUM(total_winnings), 0) INTO v_total_rolled
    FROM public.battle_players WHERE battle_id = _battle_id;
  v_payout := (v_total_rolled * (10000 - b.house_edge_bps)) / 10000;
