...
BEGIN
  ...
  FOR cs IN SELECT * FROM public.battle_cases WHERE battle_id = _battle_id ORDER BY position LOOP
    SELECT EXISTS (SELECT 1 FROM public.case_items WHERE case_id = cs.case_id AND rarity = 'legendary') INTO v_has_empire_legendary;
    SELECT EXISTS (SELECT 1 FROM public.case_items WHERE case_id = cs.case_id AND rarity = 'mythic') INTO v_has_empire_mythic;
    v_has_duel_legendary := v_has_empire_legendary;
    SELECT EXISTS (SELECT 1 FROM public.case_items WHERE case_id = cs.case_id AND rarity = 'common') INTO v_has_duel_common;

    FOR pl IN SELECT * FROM public.battle_players WHERE battle_id = _battle_id ORDER BY slot LOOP
      v_special := 'none';
      IF (v_has_empire_legendary OR v_has_empire_mythic) AND random() < 0.01 THEN
        v_special := 'empire';
      END IF;
      IF v_special = 'none' AND (v_has_duel_legendary OR v_has_duel_common) AND random() < 0.03 THEN
        v_special := 'duel';
      END IF;

      IF v_special = 'empire' THEN
        -- Empire pool: legendary/mythic only, with legendary slightly favored (60/40) when both exist.
        IF v_has_empire_legendary AND v_has_empire_mythic THEN
          v_target_rarity := CASE WHEN random() < 0.60 THEN 'legendary' ELSE 'mythic' END;
        ...
        WHERE ci.case_id = cs.case_id AND ci.rarity = v_target_rarity
        ORDER BY random() * GREATEST(ci.weight, 0) DESC
        LIMIT 1;
      ...
      ELSIF v_special = 'duel' THEN
        -- Duel pool: exactly legendary/common rarity choice 50/50 when both exist.
        IF v_has_duel_legendary AND v_has_duel_common THEN
          v_target_rarity := CASE WHEN random() < 0.50 THEN 'legendary' ELSE 'common' END;
        ELSIF v_has_duel_legendary THEN
          v_target_rarity := 'legendary';
        ELSE
