...
BEGIN
  ...
  -- Total VALUE rolled by the winning team (this is what gets shared)
  ...
  -- Each seat on the winning team is entitled to this share (bots forfeit theirs).
  share := team_total_value / GREATEST(team_size_actual, 1);

  -- pot_payout now stores the total team value (so UI shows "winnings" not entry pot).
  ...
  -- For each winning HUMAN: give them their OWN rolled items up to `share` value,
  -- top up the rest as coin voucher. Then handle borrow repayment.
  DECLARE
    w record; r record;
    given bigint; remainder bigint; debt bigint; net_voucher bigint;
  ...
  BEGIN
    ...
    LOOP
      given := 0;
      -- Award this player's own rolled items, cheapest first, up to their share
      FOR r IN
      ...
      -- Repay borrow first from the voucher remainder
      debt := w.borrowed;
      IF debt > 0 THEN
      ...
      -- Pay leftover as coin voucher
      net_voucher := remainder;
      IF net_voucher > 0 THEN
        UPDATE public.profiles SET coins = coins + net_voucher, total_won = total_won + net_voucher
        WHERE id = w.user_id;
      END IF;

      -- Any leftover debt: liquidate awarded items cheapest-first
      IF debt > 0 THEN
