-- Ensure place_bet can always be replaced regardless prior RETURN signature drift,
-- then normalize roulette aliases so roulette bets never hit "Unknown game".
DROP FUNCTION IF EXISTS public.place_bet(text, bigint, boolean, numeric, jsonb);
...
BEGIN
  ...
  IF _normalized_game NOT IN ('dice','coinflip','limbo','chicken','plinko','pump','snakes','moles','prediction','roulette') THEN
    IF ((_details ? 'bets') OR (_details ? 'betMap') OR (_details ? 'bet_map'))
      AND ((_details ? 'winning') OR (_details ? 'winningNumber') OR (_details ? 'winning_number')
      OR (_details ? 'spin') OR (_details ? 'wheel') OR (_details ? 'color')) THEN
      _normalized_game := 'roulette';
