...
BEGIN
  ...
  -- Per-winner share = payout split equally across winning team (including bots — bots' shares are forfeited)
  ...
  -- Award each winning HUMAN: their own rolled items + their fair share of pot (minus their borrow)
  DECLARE
    w record; r record; debt bigint;
  ...
  BEGIN
    FOR w IN SELECT * FROM public.battle_players
      ...
      ORDER BY slot
    LOOP
      -- Insert items this player rolled
      FOR r IN
      ...
      -- Pay coin share, less any borrowed amount
      ...
      -- If borrowed exceeded share, repay remainder by auto-selling cheapest items from this battle
